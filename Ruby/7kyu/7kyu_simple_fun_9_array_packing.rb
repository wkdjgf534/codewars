...
#   arr.reverse_each.reduce { |a, b| a << 8 | b }
# end

def array_packing(arr)
  arr.map { |n| format('%.8b', n) }.reverse.join.to_i(2)
end

p array_packing([24, 85, 0])

describe '#array_packing' do
  it 'returns a number after conversion of an array' do
    expect(array_packing([24, 85, 0])).to eq(21_784)
  end
end
